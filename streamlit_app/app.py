...
# --- UI 수정: 페이지 제목 변경 ---
st.set_page_config(page_title="Handwriting Digit Recognition & Retraining", layout="centered")
# --- UI 수정: 메인 제목 변경 ---
st.title(":writing_hand: 실시간 손글씨 피드백 학습 MLOps")
...
# ----- Sidebar: 메트릭 카드만 유지 -----
# --- UI 수정: 사이드바 제목 변경 ---
st.sidebar.header(":bar_chart: 모델 성능 지표")
...
# ----- Main: 업로드/피드백 -----
# --- UI 수정: 앱 사용 설명 추가 ---
st.subheader(":camera: 숫자 이미지 업로드 및 판별")
st.markdown("이미지를 업로드하고 **정답/오답 여부**를 알려주시면, 모델 재학습 데이터로 활용됩니다.")
st.markdown("---")


name = st.text_input("사용자 이름 (필수)", value="")
upload = st.file_uploader("숫자 손글씨 이미지 업로드 (0–9)", type=["png","jpg","jpeg"])
...
if upload and name.strip():
    ...
    st.image(pil_orig, caption="업로드된 이미지 (원본)", width=200)
    ...
    st.subheader(":magic_wand: 모델 예측 결과")
    c1, c2 = st.columns(2)
    c1.metric("예측 숫자", f"{pred}")
    c2.metric("확률 (신뢰도)", f"{conf*100:.2f}%")
    ...
    st.subheader(":speech_balloon: 예측 결과가 정확한가요?")
    col_ok, col_ng = st.columns(2)
    btn_ok = col_ok.button("✅ 맞았어 (정답 데이터로 저장)")
    btn_ng = col_ng.button("❌ 틀렸어 (정답 입력 후 저장)")
    ...
    if btn_ok:
        ...
        st.success(f"✅ 판별 결과가 정답으로 확인되어 백업 저장소에 저장되었습니다. (라벨: {true_label})")
    ...
    if "wrong_label_form_active" in st.session_state and st.session_state["wrong_label_form_active"]:
        ...
        with st.form("confirm_wrong_label"):
            st.warning(f"모델은 **{info['pred']}**로 예측했습니다.")
            true_label = st.number_input("실제 정답(0~9)을 입력해 주세요", min_value=0, max_value=9, value=info["pred"], step=1)
            submitted = st.form_submit_button("정답 확정 및 저장 (재학습 데이터로 사용)")
            if submitted:
                fname = f"{info['ts']}_{info['name']}_{info['uid']}.png".replace(",", "_").replace(" ", "_")
                ...
                _append_csv(LOG_UPLOADS,
                    ...
                    "ts,datetime,name,filename,pred,confidence,is_correct,true_label")
                st.success(f"💾 정답 **{true_label}**로 확정되어 재학습 데이터 및 백업 저장소에 저장되었습니다.")
...
# ----- Main 하단: 성능 히스토리 그래프 -----
st.markdown("---")
st.subheader(":calendar: 모델 성능 변화 추이 (7일)")
...
if hist is not None and len(hist):
    ...
    st.info("아직 성능 기록이 없습니다. 다음 재학습 후 표시됩니다.")

st.caption("""
- **재학습**은 매일 11:50(KST)에 1회 수행되며, 새 버전의 모델이 자동으로 반영됩니다.
- **꺾은선 그래프**는 'Clean' 데이터셋 기준 Accuracy(초록)와 Loss(빨강)를 지난 7일간 보여줍니다.
- **Robust 지표** (증강 테스트: 회전±15° + 노이즈 0.15)는 사이드바에서 확인 가능합니다.
- **업로드된 데이터**는 정답일 경우 백업(`backupdata/<label>/`), 오답일 경우 재학습(`data/<label>/`) 및 백업 저장소에 저장되어 모델 성능 개선에 활용됩니다.
""")